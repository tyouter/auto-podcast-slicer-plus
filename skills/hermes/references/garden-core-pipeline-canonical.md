...
> 弃用旧 `auto-podcast-slicer`（Watcher / `garden clip` CLI / production_protocol / 容器双通道）。
> 执行层**唯一标准 = garden_core 纯库**（`auto-podcast-slicer-plus/src/garden_core`）。本文件是重建后的权威知识。
...
**转录走进程内 `FunASRLocal`**（`from garden_core.stage_asr import FunASRLocal`，`device="cuda"`）：`funasr.AutoModel` 直接在 GPU 加载 Paraformer+VAD+Punc+SPK，AutoModel 内部分块。零网络 / 零 server / 零 503。MCP backends（`funasr_backend.py` + `funasr_mcp_backend.py`，~532 行）已删除（commit c956964）——不用搬旧项目的 `transcribe_chunked`，也不再有 `mcp_url`/`sse` 那套坑。跑任何 garden_core python 入口走 `scripts/run_garden.bat`（受控 PATH 基座，不继承调用者环境）。
...
- **API key**：`LLMClient` 读 `os.environ["DEEPSEEK_API_KEY"]`，garden conda env **不含此变量**。入口脚本必须在 `main()` 开头从 Hermes `.env` 注入：`open(r"D:\Hermes\.env").read().split("DEEPSEEK_API_KEY=***)[1...。
...
## 两个 video-clip skill 的关系（重建背景）
顶层 `skills/video-clip/`（50+ references，偏底层渲染/字体/VLM提取/调试）与本 `media-production/video-clip/`（24 references，偏工作流/转录/production）是**两套不同的 references**，部分同名部分独有。合并前必须逐文件盘点（盘点见 `D:\Hermes\projects\video-production-line\REFERENCES_AUDIT.md`）。⚠️ **删 skill/目录前必须 ls 完整内容确认独有文件**，不能凭「known-pitfalls 摘要覆盖」就判定零损失——顶层有 30+ 独有底层渲染文档。
> 重建已于 2026-06 完成：顶层 skill 整体归档（`projects/video-production-line/archive/legacy-video-clip-skill/`），media-production 成唯一 video-clip，references 清理到 14 个，旧管线/AI导演/风格复刻已移出。
...
- **排查法**：竖版字幕偏大/悬空时，对照老项目 `auto-podcast-slicer/pipeline/subtitle_style.py` 的 `_expand_mold_orientation`（ref_height）+ `margin_v` property（content_bottom），走 Reasonix 移植等价修复。
