...
```python
# 通过 MCP FunASR server（Windows CUDA）
vad_result = mcp_funasr_get_voice_activity_segments(audio_path="<project>/full_audio.mp3")
...
# 2. Windows MCP 重转录（音频路径用绝对路径）
mcp_funasr_start_speech_transcription(
    audio_path="<project>/gap_audio.mp3"
)
...
# 2. MCP 重转录（Paraformer-large）
mcp_funasr_start_speech_transcription(
    audio_path="<project>/first_Nmin.mp3"
)
...
| MCP `sentence_info` | ✅ ms 级 | ⚠️ 粗胚 | 缺口回填 |
...
- 🔴 缺口：5 段（2421-2468s，45.5 秒）→ 「灵魂与成功」段落完全缺失
  - **修复**：提取 50s 音频 → MCP 重转录 → 合并入 13 句 → 续接处零断点
