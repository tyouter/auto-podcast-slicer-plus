...
# ✅ 正确 — 绝对路径
sources:
  transcript: <project>/transcript.json  # → 转录条目: 1481
```
...
- **转录走 garden_core 的进程内 ASR**（`from garden_core.stage_asr import FunASRLocal`，`device="cuda"`）：在 GPU 上直接加载 Paraformer+VAD+Punc，模型内部自动分块，无需独立的转录 server。无 GPU 时退回 CPU（慢）。
...
调参流程见 [style-tuning-workflow.md](references/style-tuning-workflow.md)。
...
勘误表不是实验田——每条条目来自用户的实际收听验证。怀疑条目有问题时，问用户，不自己动手改。
...
成品视频在通过 IM / 通道分发前，通常需要压缩到通道允许的大小（常见上限 ≤30MB），并附一张封面帧：
...
压缩后用你所在平台的 IM / 通道发送 `compressed.mp4`，并随附封面帧 `cover.jpg`。多数通道要求同时提供封面，且对上传大小有限制（常为 ≤30MB）。
...
**方法**：
1. VAD 获取全量语音段 → 用 FunASR 的 VAD 能力
