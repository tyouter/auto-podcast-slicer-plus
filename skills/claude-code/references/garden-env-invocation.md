...
2. **「numpy 要降到 1.26.4」（旧 setup 脚本干的事）→ 不需要**。实测 numpy 2.4.6 + funasr 1.3.9 兼容，import 正常。
...
- **shell export PATH 持久污染**：shell 的 env 跨调用保持。我做"精简 PATH 实验"时 `export PATH=只含garden+System32` 替换掉了 `/usr/bin` → 后续 cat/rm/tail/write_file 全 `command not found`（git-bash 工具都在 /usr/bin）。恢复：`export PATH="/usr/bin:/bin:/usr/local/bin:/mingw64/bin:/c/Windows/System32:/c/Windows:$PATH"`。**预防：改 PATH 一律用子 shell `( )` 隔离，且 prepend 不替换。**
...
```
模型走 modelscope 共享缓存（`~/.cache/modelscope`，不重下）。中间 joblib `_count_physical_cores` 的 traceback 是无害 noise（fallback 逻辑核），转录照常完成。

**架构结论**：进程内 `FunASRLocal` 已整条替代 funasr 的 MCP backend（`FunASRMCPBackend` + `funasr_backend.py` 手写 `_MCP` + server + 8000 端口）。MCP backend 原本面向「ASR 服务跑在专用远程主机」的场景；本地单机进程内加载更简单可靠，无需起 server。`FunASRLocal` 现已落在 `src/garden_core/stage_asr/funasr_local.py`。

代价（trade-off）：进程内 ASR 要求**调 `run_from_audio` 的进程本身是 garden env python**（带上面那串 DLL+ffmpeg PATH）。建议在激活的 garden env 中运行 python 入口（activate 后 PATH 自带 DLL + ffmpeg 目录）。
