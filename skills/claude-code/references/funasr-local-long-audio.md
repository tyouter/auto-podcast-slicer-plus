...
长音频转录的 **OOM 防护是 garden_core 代码层的职责**——分块 + 显存释放内建在 `FunASRLocal` 里，**由代码层保证不 OOM，而非运行时盯着、OOM 了再补救**。这是「发现反复出错就代码化、别手动盯」原则在转录上的落地。

## 为什么需要

`FunASRLocal.transcribe()` 早期版本一次性把整段音频喂 `AutoModel.generate(input=audio.path)`，靠 `batch_size_s` 内部分批。60 秒 smoke 没事，但长音频（如 86 分钟播客）会重演早期的坑：**约 8 个 5 分钟块后 CUDA 显存累积泄漏 → `CUDA error: unknown error` / OOM**。
